...
import { ChevronRight, Check, ShoppingCart, Send } from "lucide-react";
...
const Field = ({ name, label, value, onChange, type = "text", required = false }: FieldProps) => (
  <div>
    ...
    <div data-testid="product-detail-page">
      ...
      <div className="border-b border-ink-800/80">
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 py-5 flex items-center gap-2 font-mono text-[11px] uppercase tracking-wider text-ink-400">
          <Link href="/" className="hover:text-signal-500">Home</Link>
          <ChevronRight className="w-3 h-3" />
          <Link href="/products" className="hover:text-signal-500">Products</Link>
          <ChevronRight className="w-3 h-3" />
      ...
      <section className="py-16 lg:py-24 border-b border-ink-800/80">
        <div className="max-w-[1440px] mx-auto px-6 md:px-12 lg:px-16 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16">
          ...
          <Reveal className="lg:col-span-5" delay={0.15}>
            ...
            <h1 className="font-display text-4xl sm:text-5xl lg:text-6xl font-semibold tracking-tighter mt-4 leading-[1.02] text-balance">
              {product.name}
            </h1>
            ...
            <div className="mt-10 flex flex-wrap gap-3">
              <a
                ...
                data-testid="amazon-buy"
                className="group inline-flex items-center gap-2 bg-signal-500 hover:bg-signal-600 text-white px-7 py-4 text-xs font-mono uppercase tracking-[0.2em]"
              >
                <ShoppingCart className="w-4 h-4" /> Buy on Amazon
              </a>
              <a
                href="#quote"
                data-testid="scroll-to-quote"
              ...
              <ul className="mt-8 grid grid-cols-1 md:grid-cols-2 gap-px bg-ink-800/80 border border-ink-800/80">
                {product.features.map((f) => (
                  <li key={f} className="bg-ink-950 p-5 flex items-start gap-3">
                    <span className="w-5 h-5 grid place-items-center bg-signal-500 mt-0.5 shrink-0">
                ...
                <tbody>
                  {product.specs.map(([k, v], i) => (
                    <tr key={k} className={i % 2 === 0 ? "bg-ink-950" : "bg-ink-900/40"}>
                      <td className="py-4 px-5 text-ink-400 uppercase text-[11px] tracking-wider w-1/3 border-r border-ink-800/80">
                        {k}
                      </td>
                ...
                <div>
                  ...
                  <ul className="mt-6 space-y-3">
                    {product.applications.map((a) => (
                      <li key={a} className="flex items-center gap-3 text-sm text-ink-200">
                        <span className="w-1.5 h-1.5 bg-signal-500" /> {a}
                      </li>
                    ))}
                  </ul>
                </div>
                <div>
                  ...
                  <div className="mt-6 flex flex-wrap gap-2">
                    {product.industries.map((ind) => (
                      <span key={ind} className="px-3 py-1.5 border border-ink-800 font-mono text-[11px] tracking-wider text-ink-200">
                        {ind}
                      </span>
                    ))}
                  </div>
            ...
            <Reveal>
              ...
              <ul className="mt-8 grid grid-cols-1 sm:grid-cols-2 gap-3">
                {product.deliverables.map((d) => (
                  <li key={d} className="flex items-center gap-3 border border-ink-800 px-5 py-4 text-sm text-ink-100">
                    <Check className="w-4 h-4 text-signal-500" /> {d}
                  </li>
                ))}
              </ul>
            </Reveal>
            ...
            <div className="lg:sticky lg:top-28 border border-ink-800 bg-ink-900/40">
              ...
              <form onSubmit={onSubmit} data-testid="quote-form" className="p-6 space-y-4">
                ...
                <button
                  ...
                  type="submit"
                  disabled={submitting}
                  className="group w-full inline-flex items-center justify-center gap-2 bg-signal-500 hover:bg-signal-600 disabled:opacity-60 text-white px-6 py-4 text-xs font-mono uppercase tracking-[0.2em] transition-colors"
                >
                  {submitting ? "Sending..." : <><span>Submit Request</span> <Send className="w-4 h-4" /></>}
                </button>
              </form>
            </div>
          </aside>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              {related.map((p) => (
                <Link
                  ...
                  className="group block border border-ink-800 hover:border-signal-500 transition-colors"
                >
                  ...
                  <div className="p-6">
                    <h3 className="font-display text-xl font-medium tracking-tight group-hover:text-signal-500 transition-colors">
                      {p.name}
                    </h3>
